...
export default function VoiceChatPopup({
  ...
  const autoSendRef = useRef<boolean>(true);
    ...
    try {
      if (useRealtime) {
        ...
        const controller = await assemblyaiRealtime.startRealtimeTranscription({
          ...
          onFinal: (t) => {
            setTranscript(t);
            if (autoSendRef.current) pushFinalTranscript(t);
            else setPendingUser(t);
          },
          onError: (e) => {
        ...
        // start AssemblyAI recorder with simple VAD; it will stop and upload on silence
  ...
  function toggleAlwaysListen() {
    ...
    if (next && visible && !recorderRef.current) startRecording();
  }
  ...
  function toggleAutoSend() {
    const next = !autoSendRef.current;
