...
def build_graph(agent_type="ruhuna"):
    """
    Builds an agent graph for either Ruhuna or UGC agent.

    Args:
        agent_type (str): Either "ruhuna" or "ugc" to determine tool access.
    
    Returns:
        graph (StateGraph): Compiled graph for the selected agent.
    """
    import os
...
#     # Set your OpenRouter API key from env
#     os.environ["OPENAI_API_KEY"] = os.getenv("OPENROUTER_API_KEY")
#     os.environ["OPENAI_BASE_URL"] = "https://openrouter.ai/api/v1"

#     primary_llm = ChatOpenAI(
#         model=TOOLS_CFG.primary_agent_llm,  # e.g. "openai/gpt-4o-mini" or "anthropic/claude-3.5-sonnet"
#         temperature=TOOLS_CFG.primary_agent_llm_temperature
#     )
